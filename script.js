...
const stepsData = [
  // STEP 1: Intro (Clock World)
  {
    ...
    story: {
      ...
      ko: "시계의 세계에서 분침은 나무를 바라보며 불안해했습니다.\n변화를 막고자 초침에게 หยุดเดิน해달라고 부탁했지만,\n나무가 계절을ผ่าน 성장하기 위해서는 시간이 계속 흘러가야만 했습니다...",
    ...
    options: [
      ...
      { text: { th: "ตกใจนิดหน่อย แต่ก็ตระหนักว่าเวลาในใจเรายังเดินต่อไป", ko: "조금 놀랐지만 내 마음속 시간은 계속 흘러감을 깨닫는다", en: "Slightly surprised, but realize time inside still flows", ja: "少し驚いたが、心の中 me の時間は流れ続けていると気づく" }, card: "c9", pts: 2 },
  ...
  // STEP 2: Spring
  {
    ...
    options: [
      { text: { th: "รู้สึกใจหาย เพราะความงามในวันเก่ากำลังจะร่วงโรยไป", ko: "지난날의 아름다움이 시들어가는 것 같아 마음이 아프다", en: "Feel nostalgic, as past beauty fades away", ja: "過去の美しさが 枯れていくようで切ない" }, card: "c1", pts: 2 },
      ...
      { text: { th: "ปล่อยให้มันเป็นไป เพราะมันเป็นเรื่องธรรมดาของชีวิต", ko: "삶의 자연스러운 순리이므로 그저 담담히 받아들인다", en: "Accept it calmly, as it is a natural part of life", ja: " Over the natural flow of life, accept it calmly" }, card: "c10", pts: 1 }
    ]
  },
  // STEP 3: Summer
  {
    season: { th: "ฤดูร้อน: แสงสว่าง ฝนโปรย และความจริง", ko: "여름: 빛, 여우비, 그리고 진실", en: "Summer: Sunlight, Sunshower & Truth", ja: "夏: 光、狐の嫁入り、และ真実" },
    story: {
    ...
    question: {
      ...
      ja: "快晴の下を歩いているとき、突然狐の嫁入り（天気雨）ที่降ってきたらどうしますか？"
    },
    options: [
      ...
      { text: { th: "ยิ้มรับสายฝนและความเย็น คิดว่าฝนตกแดดออกก็สวยดี", ko: "비와 햇살이 함께하는 풍경도 아름답다고 생각하며 미소 짓는다", en: "Smile, finding beauty in rain and sunshine together", ja: "雨と sun が共存する風景も美しいと微笑む" }, card: "c6", pts: 1 },
  ...
  // STEP 4: Autumn
  {
    ...
    story: {
      ...
      ja: "ペンで書かれた past は消せないため、ただ振り返るだけです。\n陽光が眩しい昼間には、いつも傍で見守ってくれる「2つの星」の存在すら忘れてしまいが치です..."
    },
    question: {
  ...
  // STEP 5: Winter
  {
    ...
    options: [
      ...
      { text: { th: "แสงจากการปกป้องเงียบๆ ของคนที่รักเราโดยไม่หวังผล", ko: "댓가를 바라지 않고 묵묵히 나를 지켜주는 사랑의 빛", en: "The quiet, unconditional protective light of those who love us", ja: "見返りを求めず静かに見守ってくれる愛の光" }, card: "c5", pts: 2 },
      { text: { th: "แสงแห่งความอบอุ่นที่ส่องผ่านในค่ำคืนที่หนาวเหน็บ", ko: "춥고 어두운 밤을 따뜻하게 비춰주는 온기의 빛", en: "The warm light that pierces through a cold, dark night", ja: " 寒く暗い夜 festival 温かく照らすぬくもりの光" }, card: "c3", pts: 1 },
...
const cardsData = {
  c1: {
    ...
    desc: { th: "คุณเป็นคนที่ละเอียดอ่อน ลึกซึ้ง และแคร์ความรู้สึกของผู้คนรอบข้าง คุณเข้าใจดีว่าการจากลาเป็นส่วนหนึ่งของการเติบโต", ko: "당신은 섬세하고 깊은 마음을 가진 사람입니다. 이별 또한 성장의 일부임을 잘 이해하고 있습니다.", en: "You have a delicate and deep heart, understanding that farewells are a part of growing up.", ja: "あなたは繊細で deep な心を持つ人です。別れも成長の一部であることをよく理解しています。" }
  },
  c2: {
  ...
  c3: {
    ...
    desc: { th: "คุณเป็นคนทุ่มเทให้ความรัก ผูกพันกับสถานที่และความทรงจำอันอบอุ่นที่เคยเกิดขึ้น", ko: "당신은 사랑에 진심이며, 따뜻했던 장소와 기억을 깊이 간직하는 사람입니다.", en: "You give your heart sincerely, holding deep affection for places and warm memories.", ja: "あなたは愛に真実であり、 warm だった場所と記憶を深く大切にする人です。" }
  },
  c4: {
...
function renderStep() {
  ...
  // Default: Show Story Page First
  showPage('storyPage');
}

function startTest() {
    // 1. รีเซ็ตค่าตัวแปรและคะแนนเริ่มต้นของแบบทดสอบ
    stepIndex = 0;
    scores = { c1: 0, c2: 0, c3: 0, c4: 0, c5: 0, c6: 0, c7: 0, c8: 0, c9: 0, c10: 0 };
    
    // 2. จัดการเปลี่ยนหน้าจอ (ซ่อนหน้าคำเตือน และแสดงหน้าเนื้อเรื่องด้วยอนิเมชัน)
    ...
    // 3. เริ่มต้นเรนเดอร์เนื้อหาหรือคำถามสเตปแรก
    renderStep();
}
...
function restartTest() {
  ...
  if (seasonTitleEl) {
    seasonTitleEl.innerText = "WINTER EDITION";
  }
  showPage('page-cover'); // กลับไปหน้าปก
}
...
function showFinalCard() {
  ...
  // 🌟 จุดสำคัญ: ต้องมีแท็ก <img> ตรงนี้ เพื่อให้มันดึงรูปจาก card.img มาโชว์
...
function changeLang(lang) {
  currentLang = lang;
  if (stepIndex < stepsData.length) {
    // Re-render text according to current active view
    const step = stepsData[stepIndex];
    document.getElementById('seasonTitle').innerText = step.season[currentLang];
    document.getElementById('storyText').innerText = step.story[currentLang];
    document.getElementById('questionText').innerText = step.question[currentLang];
